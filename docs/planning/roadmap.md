...
3. ~~Versioned IPC framing and session authentication.~~ Completed with bounded frames,
   supervisor-bound identity, constant-time token checks, stable responses, and socket
   tests.
4. Supervisor Unix listener, peer credentials, deadlines, replay protection, and
   revocation.
5. Filesystem write/apply broker with review-digest binding and rollback.
6. Process sandbox, egress proxy, and resource limits.
7. Opaque credential flow and production audit chain.
8. Code-OSS shell spike and permission center.
9. Agent patch/review/apply workflow.

## Runnable application track

The installable local application is now available as `guardrails-ide`. It provides a
bundled VS Code-inspired shell, protected workspace index, brokered source viewing, and
live audit panel. It binds to localhost and requires no Node.js runtime. Read-only is a
security gate, not an omitted UI feature: editing, terminals, extensions, and agents
will be exposed only when their roadmap brokers pass end-to-end sandbox tests.
