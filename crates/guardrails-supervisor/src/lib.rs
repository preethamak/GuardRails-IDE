...
impl Supervisor {
    ...
    pub fn serve_once(&self) -> Result<Option<String>, SupervisorError> {
        ...
        let response = match message {
            Ok(ClientMessage::Handshake(request))
                if request.protocol_version != PROTOCOL_VERSION =>
            {
                ServerMessage::Rejected(RejectionResponse {
                    code: RejectionCode::UnsupportedProtocol,
                })
            }
            Ok(ClientMessage::Handshake(request))
                if request.launch_token != self.launch_token
                    || !valid_principal(&request.principal_id) =>
            {
                ServerMessage::Rejected(RejectionResponse {
                    code: RejectionCode::AuthenticationFailed,
                })
            }
        ...
        let response = match read_message(&mut reader)? {
            ...
            _ => ServerMessage::Rejected(RejectionResponse {
                code: RejectionCode::InvalidRequest,
            }),
        };
        write_message(&mut stream, &response)?;
...
mod tests {
    ...
    fn request(token: &str, principal: &str, version: u16) -> String {
        format!(
            "{{\"kind\":\"handshake\",\"protocol_version\":{version},\"launch_token\":\"{token}\",\"principal_id\":\"{principal}\"}}\n"
        )
    }
